...
def get_job_queries(role, level):
    ...
    if not safe_role and not safe_level:
        return ["Jobs"]

    base_titles = ROLE_QUERY_ALIASES.get(safe_role, [safe_role or "Jobs"])
    prefixes = LEVEL_PREFIXES.get(safe_level, [safe_level] if safe_level else [""])
...
def build_external_links(job_queries):
    safe_queries = [q for q in (job_queries or []) if isinstance(q, str) and q.strip()]
    joined_query = " OR ".join(safe_queries) if safe_queries else "jobs"
